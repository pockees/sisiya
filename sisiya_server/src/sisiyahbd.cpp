...
void setDefaults(void)
{
	...
	confFile.setDefault("CLIENT_CONF","/opt/sisiya_checks/sisiya_client.conf");
}
...
//	systemsConfFile.setFileName(systemsConfFileName);
	...
	pidFile << serverPID;
	pidFile.close();
	syslog(LOG_INFO,"closed PID file %s",confFile.getString("PID_FILE").c_str());
...
void *t_server(void *arg)
{
	...
	switch(retcode) {
		case 0 : // OK
			break;
		case ESRCH :
			syslog(LOG_ERR,"no thread could be found with thread ID=%d to detach! Etrror code=%d message=%s",tid,retcode,strerror(retcode));
			return NULL;
			break;
		case EINVAL :
			syslog(LOG_ERR,"thread with ID=%d is already in detached state! Etrror code=%d message=%s",tid,retcode,strerror(retcode));
			return NULL;
			break;
		default :
			syslog(LOG_ERR,"could not detach thread with ID=%d! Etrror code=%d message=%s",tid,retcode,strerror(retcode));
			return NULL;
			break;
	}
	childPID=tid;
	if(loglevel > 2)
		syslog(LOG_INFO,"thread with ID=%d detached itself.",(int)tid);    
...
bool lockMutex(pthread_mutex_t &mutex)
{
	...
	switch(retcode) {
		case 0 :
			...
			//break;
		case EINVAL :
			syslog(LOG_ERR,"lockMutex: child server(%d) : the mutex is not properly inituialized!",childPID);
			break;
		case EDEADLK :
			syslog(LOG_ERR,"lockMutex: child server(%d) : the mutex is already lock by the calling thread!",childPID);
			break;
		default :
			syslog(LOG_ERR,"lockMutex: child server(%d) : got unknown error code=%d while trying to lock the mutex!",childPID,retcode);
			break;
	}
	return false;
}
...
bool unlockMutex(pthread_mutex_t &mutex)
{
	...
	switch(retcode) {
		case 0 :
			...
			//break;
		case EINVAL :
			syslog(LOG_ERR,"unlockMutex: child server(%d) : the mutex is not properly inituialized",childPID);
			break;
		case EPERM :
			syslog(LOG_ERR,"unlockMutex: child server(%d) : the calling thread does not own this mutex!",childPID);
			break;
		default :
			syslog(LOG_ERR,"unlockMutex: child server(%d) : got unknown error code=%d while trying to unlock the mutex!",childPID,retcode);
			break;
	}
	return false;
}
...
bool condWait(pthread_cond_t &cond,pthread_mutex_t &mutex)
{
	...
	syslog(LOG_ERR,"condWait: child server(%d) : got unknown error while waiting on the conditional variable!",childPID);
	return false;
...
bool condSignal(pthread_cond_t &cond)
{
	if(pthread_cond_signal(&cond) == 0)
		return true;
	// According to the man page this function never returns a error code.
	syslog(LOG_ERR,"condSignal: child server(%d) : got unknown error while signalling for the conditional variable!",childPID);
	return false;
}
...
bool condBroadcast(pthread_cond_t &cond)
{
	if(pthread_cond_broadcast(&cond) == 0)
		return true;
	// According to the man page this function never returns a error code.
	syslog(LOG_ERR,"condBroadcast: child server(%d) : got unknown error while broadcasting the conditional variable!",childPID);
	return false;
...
{
	if(argc != 3 ) {
		cerr << "Usage : " << argv[0] << " sisiyahbd.conf sisiyahbd_systems.conf" << endl;
		return 1;
	}
