...
class A:public B {
      public:
	void f() {
		throw SQLException("class exception");
}};
...
int main(void)
{
	...
	conn[0] =
	    dm.getConnection("edbc:postgresql://localhost/template1",
			     "postgres", "post123654");
	conn[1] =
	    dm.getConnection("edbc:mysql://localhost/mysql", "mysql",
			     "mysql123654");
...
//      B *a1=new A; try{ a1->f(); f1();} catch(SQLException &e) { cout << "caught SQLException message=" << e.getMessage() << endl; return 0;}
	...
	for (i = 0; i < max; i++) {
		...
		cout << "Database major version    	= " << dmd[i]->
		    getDatabaseMajorVersion() << endl;
		cout << "Database minor version    	= " << dmd[i]->
		    getDatabaseMinorVersion() << endl;
		cout << "Database sub   version    	= " << dmd[i]->
		    getDatabaseSubVersion() << endl;
		cout << "Database product name     	= " << dmd[i]->
		    getDatabaseProductName() << endl;
		cout << "Database product version	= " << dmd[i]->
		    getDatabaseProductVersion() << endl;

		autoCommit = conn[i]->getAutoCommit();
		cout << "Auto commit 			= " << autoCommit
		    << endl;
		if (autoCommit == false) {
			cout << "Auto commit is false. Setting to true ..."
			    << endl;
			conn[i]->setAutoCommit(true);
			autoCommit = conn[i]->getAutoCommit();
			cout << "Now auto commit is		= " <<
			    autoCommit << endl;
		}
		sql =
		    "create  table test_table(id int,d double precision,str char(32),str2 char(16))";
		...
		for (j = 0; j < 10; j++) {
			sprintf(str,
				"insert into test_table (id,d,str) values(%d,%lf,'This is the %d th row')",
				j + 1, 2.3 * j, j + 1);
			cout << "Executing Query :[" << str << "]" << endl;
			if (stmt[i]->executeUpdate(str) != 1) {
				cout <<
				    "Query execution FAILED: Insert must produce exactly 1"
				    << endl;
			}
		}
		...
		if (rs[i] = stmt[i]->executeQuery(sql)) {
			...
			cout << "Column count 	= " << rsmd[i]->
			    getColumnCount() << endl;
			cout << "Row count 	= " << rsmd[i]->
			    getRowCount() << endl;
			rs[i]->beforeFirst();
			while (rs[i]->next()) {
