...
void PostgreSQL_Connection(char *host_str, char *user_str,
			   char *password_str, char *db_str,
			   unsigned int port1 = 5432)
{
...
bool connect(void)
{
	...
	cout << "connect: Connecting to " << user << "/" << db << "@" <<
	    host << "..." << endl;

	if (isdigit(host[0])) {
		if (snprintf
		    (conninfo, MAX_STR,
		     "hostaddr=%s dbname=%s user=%s password=%s", host, db,
		     user, password) == -1)
			return false;
	} else {
		if (snprintf
		    (conninfo, MAX_STR,
		     "host=%s dbname=%s user=%s password=%s", host, db,
		     user, password) == -1)
			return false;
	}
	...
	for (int i = 0; i < PQnfields(result); i++) {
		cout << "i=" << i << " type=" << PQftype(result,
							 i) << " value=" <<
		    PQgetvalue(result, 0, i) << endl;
	}
	return true;
}
...
void db_close(void)
{
	cout << "db_close: Disconnecting from " << user << "/" << db << "@"
	    << host << "..." << endl;
	PQfinish(pg_conn);
}
...
int main(void)
{
	PostgreSQL_Connection("localhost", "postgres", "post123654",
			      "template1");
	if (connect())
		cout << "Connected" << endl;
	else {
