...
*/
bool getSystemUpdateChangeTimes(Connection *conn,int systemID,string &updateTime,string &changeTime)
{
//	if(loglevel > 2)
		syslog(LOG_INFO,"getSystemUpdateChange: Getting update and change times for systemID=%d",systemID); 
...
//	if(loglevel > 2)
		syslog(LOG_INFO,"getSystemUpdateChange: Getting update and change times for systemID=%d sql=[%s]",systemID,sql.c_str()); 
	Statement *stmt;
	try {
		stmt=conn->createStatement();
	}
	...
	if(rs == NULL) {
		...
		if(loglevel > 2)
			syslog(LOG_INFO,"getSystemUpdateChangeTimes: Query [%s] executon OK",sql.c_str());
...
//	if(loglevel > 2)
		syslog(LOG_INFO,"getSystemUpdateChange: Got update=[%s] time for systemID=%d",updateTime.c_str(),systemID); 

	sql=string("select changetime from systemservicestatus where systemid=")+toString(systemID)+string(" order by changetime desc");
syslog(LOG_INFO,"getSystemUpdateChange: Getting change time for systemID=%d sql=[%s]",systemID,sql.c_str()); 
	try {
		rs=stmt->executeQuery(sql);
	}
	catch(SQLException &e) {
	...
	if(rs == NULL) {
		...
		if(loglevel > 2)
			syslog(LOG_INFO,"getSystemUpdateChangeTimes: Query [%s] executon OK",sql.c_str());
syslog(LOG_INFO,"getSystemUpdateChange: systemID=%d sql=[%s] executed",systemID,sql.c_str()); 
	try {
		rsmd=rs->getResultSetMetaData();
	}
	catch(SQLException &e) {
...
syslog(LOG_INFO,"getSystemUpdateChange: systemID=%d sql=[%s] executed and got rsmd",systemID,sql.c_str());
	if(rsmd == NULL) {
		syslog(LOG_ERR,"getSystemUpdateChangeTimes: Could not create ResultSetMetadata object!");
		delete rs;
		delete stmt;
...
syslog(LOG_INFO,"getSystemUpdateChange: systemID=%d sql=[%s] executed row count=%ld",systemID,sql.c_str(),rsmd->getRowCount());
	if(rsmd->getRowCount() == 0) {
		//if(loglevel > 1)
		//	syslog(LOG_INFO,"getSystemUpdateChangeTimes: No records for systemID=%d in the systemservicestatus table!",systemID);
		delete rsmd;
...
//	if(loglevel > 2)
		syslog(LOG_INFO,"getSystemUpdateChange: Got update=[%s] and change=[%s] times for systemID=%d",updateTime.c_str(),changeTime.c_str(),systemID); 
	return true;
}	
...
cout << "here" << endl;
	...
	try {
		...
		cerr << "main: Error connecting to the DB! Caught SQLException:" << endl;
		cerr << e.getMessage() << endl;
		return false;
	}
...
cout << "here4" << endl;
	if(getSystemUpdateChangeTimes(conn,1,updateTime,changeTime) == false) {
		cerr << "main: Could not get update/change times!" << endl;
		delete conn;
		return 1;
	}		
cout << "here5" << endl;
	cout << "Update time = " << updateTime << " change time = " << changeTime << endl;
//	doAllTests();
	return 0;
}
