...
#ifdef DEBUG
	cout <<
	    "maker(PostgreSQL_Connection): creating a PostgreSQL_Connection ..."
	    << endl;
#endif
...
#ifdef DEBUG
		cout <<
		    "maker(PostgreSQL_Connection): calling PostgreSQL_Connection"
		    << endl;
#endif
		PostgreSQL_Connection *c =
		    new PostgreSQL_Connection(server, user, password,
					      dbname, port);
		return c;
	}
	return NULL;
...
PostgreSQL_Connection::PostgreSQL_Connection(const string host,
					     const string user,
					     const string password,
					     const string db,
...
#ifdef DEBUG
	cout << "PostgreSQL_Connection::Constructor(" << host << "," <<
	    user << "," << password << "," << db <<
...
#ifdef DEBUG
	cout <<
	    "PostgreSQL_Connection::Destructor : destructing a PostgreSQL Connection object:"
	    << this << endl;
#endif
	close();
/*
...
#ifdef DEBUG
	cout <<
	    "PostgreSQL_Connection:beginTransaction: Starting transaction..."
	    << endl;
#endif
	PGresult *result;
...
#ifdef DEBUG
		cout <<
		    "PostgreSQL_Connection:beginTransaction:  Already in transaction. Exiting without starting a transaction..."
		    << endl;
#endif
		return;
	}
...
#ifdef DEBUG
		cout <<
		    "PostgreSQL_Connection::beginTransaction: Could not begin a transaction"
		    << endl;
#endif
		/*
...
#ifdef DEBUG
		cout <<
		    "PostgreSQL_Connection::beginTransaction: Could not set transaction isolation level"
		    << endl;
#endif
		string
		    a
...
{
#ifdef DEBUG
	cout << "PostgreSQL_Connection::connect: Connecting to " << user <<
	    "/" << db << "@" << host << "..." << endl;
#endif
	...
	if (isdigit(host[0]))
		conninfo =
		    string("hostaddr=") + host + string(" dbname=") + db +
		    string(" user=") + user + string(" password=") +
		    password;
	else
		conninfo =
		    string("host=") + host + string(" dbname=") + db +
		    string(" user=") + user + string(" password=") +
		    password;
	...
	if (pg_conn == NULL) {	// could not allocate the object
		throw
		    SQLException(string
				 ("PostgreSQL_Connection::connect: Could not allocate pg_conn object!"));
		//return false;
	}
	if (PQstatus(pg_conn) == CONNECTION_BAD) {
#ifdef DEBUG
		cerr << "PostgreSQL_Connection::connect: " <<
		    PQerrorMessage(pg_conn) << endl;
#endif
		throw
		    SQLException(string
				 ("PostgreSQL_Connection::connect: Could not connect: ")
...
{
#ifdef DEBUG
	cout << "PostgreSQL_Connection:close: Disconnecting from " << user
	    << "/" << db << "@" << host << "..." << endl;
#endif
	if (!autoCommit) {
#ifdef DEBUG
		cout <<
		    "PostgreSQL_Connection:close: The connection was not in auto-commit mode. Commiting the transaction..."
		    << endl;
#endif
		commit();
	}
...
void PostgreSQL_Connection::commit(void)
{
	if (autoCommit)
		throw
		    SQLException(string
				 ("PostgreSQL_Connection::commit: The connection is in auto-commit mode."));

	PGresult *result;

#ifdef DEBUG
	cout <<
	    "PostgreSQL_Connection:commit: Commiting the transaction..." <<
	    endl;
#endif
...
#ifdef DEBUG
		cout <<
		    "PostgreSQL_Connection:commit: Not in transaction. Exiting without commit..."
		    << endl;
#endif
		throw
		    SQLException(string
				 ("PostgreSQL_Connection:commit: Not in transaction!"));
		//return;
	}
	if (autoCommit) {	// this must not happen, but I put this check anyway.
#ifdef DEBUG
		cout <<
		    "PostgreSQL_Connection:commit: autoCommit is true! Exiting without commit..."
		    << endl;
#endif
		throw
		    SQLException(string
				 ("PostgreSQL_Connection:commit: This connection is in auto commit mode!"));
		//return;
	}
...
#ifdef DEBUG
		cout <<
		    "PostgreSQL_Connection::execute: Could not commit the transaction"
		    << endl;
#endif
		string b(PQresultErrorMessage(result));
		PQclear(result);
...
void PostgreSQL_Connection::rollback(void)
{
#ifdef DEBUG
	cout <<
	    "PostgreSQL_Connection:rollback: Rolling back the transaction..."
	    << endl;
#endif
	PGresult *result;
	...
	if (autoCommit)
		throw
		    SQLException(string
				 ("PostgreSQL_Connection::rollback: The connection is in auto-commit mode."));
	if (PQtransactionStatus(pg_conn) != PQTRANS_INTRANS) {
		throw
		    SQLException(string
				 ("PostgreSQL_Connection::rollback: Not in transaction!"));
		//return;
	}
...
#ifdef DEBUG
		cout <<
		    "PostgreSQL_Connection:rollback: autoCommit is true! Exiting without rolling back..."
		    << endl;
#endif
		throw
		    SQLException(string
				 ("PostgreSQL_Connection::rollback: This connection is in auto commit mode!"));
		//return;
	}
...
#ifdef DEBUG
		cout <<
		    "PostgreSQL_Connection::rollback: Could not roll back the transaction"
		    << endl;
#endif
		string b(PQresultErrorMessage(result));
		PQclear(result);
...
{
	PostgreSQL_DatabaseMetaData *dbmd =
	    new PostgreSQL_DatabaseMetaData;
...
#ifdef DEBUG
		cout <<
		    "PostgreSQL_Connection::setAutoCommit: set autoCommit to "
		    << autoCommit << endl;
#endif
	}
}
...
void PostgreSQL_Connection::setTransactionIsolation(int level)
{
	...
	default:
		ostringstream osstr;
		osstr <<
		    "PostgreSQL_Connection::setTransactionLevel: level=" <<
