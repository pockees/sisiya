...
{ 
#ifdef DEBUG
	cout << "Constructor : Constructing a PostgreSQL_ResultSet object:" << this << endl; 
#endif
	stmt=NULL;
	pg_conn=NULL;
	result=NULL;
...
{ 
#ifdef DEBUG
	cout << "PostgreSQL_ResultSet::Destructor: destructing a PostgreSQL_ResultSet object: " << this << endl; 
#endif
	PQclear(result);
#ifdef DEBUG
	cout << "PostgreSQL_ResultSet::Destructor: destructing a PostgreSQL_ResultSet object: " << this << " OK" << endl; 
#endif
}
...
bool PostgreSQL_ResultSet::getBoolean(int columnIndex)
{
	if(columnIndex < 0 || columnIndex > rsmd->getColumnCount()) {
		ostringstream osstr;
		osstr << "PostgreSQL_ResultSet::getBoolean: columnIndex=" << columnIndex << " > columnCount=" << rsmd->getColumnCount() << ends;
		throw SQLException(osstr.str());
	}
	if(PQftype(result,columnIndex) != PostgreSQL_ResultSetMetaData::PostgreSQL_Type_Boolean) {
		ostringstream osstr;
		osstr << "PostgreSQL_ResultSet::getBoolean: columnIndex=" << columnIndex << " is not of boolean type!" << ends;
		throw SQLException(osstr.str());
	}
...
double PostgreSQL_ResultSet::getDouble(int columnIndex)
{
	if(columnIndex < 0 || columnIndex > rsmd->getColumnCount()) {
		ostringstream osstr;
		osstr << "PostgreSQL_ResultSet::getDouble: columnIndex=" << columnIndex << " > columnCount=" << rsmd->getColumnCount() << ends;
		throw SQLException(osstr.str());
	}
...
//		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_LongInt :
		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_ShortInt :
			return 1.0*atoi(PQgetvalue(result,currentRow,columnIndex));
			break;
		default :
			ostringstream osstr;
			osstr << "PostgreSQL_ResultSet::getDouble: columnIndex=" << columnIndex << " is not double or it cannot be coverted into double!" << ends;
			throw SQLException(osstr.str());
			//break;
	}
}
...
float PostgreSQL_ResultSet::getFloat(int columnIndex)
{
	if(columnIndex < 0 || columnIndex > rsmd->getColumnCount()) {
		ostringstream osstr;
		osstr << "PostgreSQL_ResultSet::getFloat: columnIndex=" << columnIndex << " > columnCount=" << rsmd->getColumnCount() << ends;
		throw SQLException(osstr.str());
	}
	switch(PQftype(result,columnIndex)) {
		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_Double :
		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_Float :
			return (float)atof(PQgetvalue(result,currentRow,columnIndex));
			break;
		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_Int :
//		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_LongInt :
		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_ShortInt :
			return (float)1.0*atoi(PQgetvalue(result,currentRow,columnIndex));
			break;
		default :
			ostringstream osstr;
			osstr << "PostgreSQL_ResultSet::getDouble: columnIndex=" << columnIndex << " is not double or it cannot be coverted into float!" << ends;
			throw SQLException(osstr.str());
			//break;
	}
}
...
int PostgreSQL_ResultSet::getInt(int columnIndex)
{
	if(columnIndex < 0 || columnIndex > rsmd->getColumnCount()) {
		ostringstream osstr;
		osstr << "PostgreSQL_ResultSet::getInt: columnIndex=" << columnIndex << " > columnCount=" << rsmd->getColumnCount() << ends;
		throw SQLException(osstr.str());
	}
	switch(PQftype(result,columnIndex)) {
		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_Double :
...
//		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_LongInt :
		...
		default :
			ostringstream osstr;
			osstr << "PostgreSQL_ResultSet::getDouble: columnIndex=" << columnIndex << " is not double or it cannot be coverted into int!" << ends;
			throw SQLException(osstr.str());
			//break;
	}
}
...
long int PostgreSQL_ResultSet::getLong(int columnIndex)
{
	if(columnIndex < 0 || columnIndex > rsmd->getColumnCount()) {
		ostringstream osstr;
		osstr << "PostgreSQL_ResultSet::getLong: columnIndex=" << columnIndex << " > columnCount=" << rsmd->getColumnCount() << ends;
		throw SQLException(osstr.str());
	}
	switch(PQftype(result,columnIndex)) {
		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_Double :
...
//		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_LongInt :
		...
		default :
			ostringstream osstr;
			osstr << "PostgreSQL_ResultSet::getDouble: columnIndex=" << columnIndex << " is not double or it cannot be coverted into long!" << ends;
			throw SQLException(osstr.str());
			//break;
	}
}
...
short int PostgreSQL_ResultSet::getShort(int columnIndex)
{
	if(columnIndex < 0 || columnIndex > rsmd->getColumnCount()) {
		ostringstream osstr;
		osstr << "PostgreSQL_ResultSet::getShort: columnIndex=" << columnIndex << " > columnCount=" << rsmd->getColumnCount() << ends;
		throw SQLException(osstr.str());
	}
	switch(PQftype(result,columnIndex)) {
		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_Double :
...
//		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_LongInt :
		case PostgreSQL_ResultSetMetaData::PostgreSQL_Type_ShortInt :
			return (short int)atoi(PQgetvalue(result,currentRow,columnIndex));
			break;
		default :
			ostringstream osstr;
			osstr << "PostgreSQL_ResultSet::getDouble: columnIndex=" << columnIndex << " is not double or it cannot be coverted into short!" << ends;
			throw SQLException(osstr.str());
			//break;
	}
...
void PostgreSQL_ResultSet::setResultSetMetaData(PostgreSQL_ResultSetMetaData *rsmd)
{
	this->rsmd=rsmd;
}
...
string PostgreSQL_ResultSet::getString(int columnIndex)
{
	if(columnIndex < 0 || columnIndex > rsmd->getColumnCount()) {
		ostringstream osstr;
		osstr << "PostgreSQL_ResultSet::getString: columnIndex=" << columnIndex << " > columnCount=" << rsmd->getColumnCount() << ends;
		throw SQLException(osstr.str());
	}
