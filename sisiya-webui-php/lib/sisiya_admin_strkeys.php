...
$table_header_fields=array(
	...
	1 => array('key' => 'keystr',	'label' => $lrb['sisiya_admin.'.$table_name.'.label.keystr']),
	2 => array('key' => 'str',	'label' => $lrb['sisiya_admin.'.$table_name.'.label.str'])
);
$fields=array(
	0 => array('field_name' => 'new_id',		'key' => 'id',		'value' => '',	'is_str' => false),
	1 => array('field_name' => 'new_keystr',	'key' => 'keystr',	'value' => '',	'is_str' => true),
	2 => array('field_name' => 'new_str',		'key' => 'str',		'value' => '',	'is_str' => true)
);
$nrows=get_setNRows($table_name);

if(isset($_POST['add']))
	execSQLWrapper('add',"insert into ".$table_name." values(".getNewID('new_id',$table_name).",'".$_POST['new_keystr']."','".$_POST['new_str']."')");
	...
	for($i=0;$i<$nrows;$i++) {
		if(isset($_POST['delete'][$i]))
			...
			execSQLWrapper('update','update '.$table_name." set keystr='".$_POST['keystr'][$i]."',str='".$_POST['str'][$i]."' where id=".$_POST['id'][$i]);
		}
}
...
if($_SESSION['is_admin'] == 't') { 
	...
	$html.='	<td><input class="text_large" type="text" name="new_keystr" value="'.getFieldValue($fields,'new_keystr').'" /></td>'."\n";
	$html.='	<td><input class="text_wide" type="text" name="new_str" value="'.getFieldValue($fields,'new_str').'" /></td>'."\n";
...
$sql_str='select id,keystr,str from '.$table_name;
...
if(!$result)
	...
	if($nrows > 0) {
		...
		for($i=$start_index;$i<$start_index + $nrecords_per_page;$i++) {
			...
			$html.='	<td><input class="text_large" type="text" name="keystr['.$i.']" value="'.$row[1].'" /></td>'."\n";
			$html.='	<td><input class="text_wide" type="text" name="str['.$i.']" value="'.$row[2].'" /></td>'."\n";
			if($_SESSION['is_admin'] == 't') { 
