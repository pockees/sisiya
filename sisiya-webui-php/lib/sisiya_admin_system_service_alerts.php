...
if($_SESSION['is_admin'] == 't') { 
	...
	$table_header_fields=array(
		...
		2=> array('key'=>'i3.str',	'label'=>$lrb['sisiya_admin.services.label.keystr']), 
		3=> array('key'=>'i.str',	'label'=>$lrb['sisiya_admin.label.status']), 
		4=> array('key'=>'i2.str',	'label'=>$lrb['sisiya_admin.alerttypes.label.keystr']), 
		5=> array('key'=>'a.enabled',	'label'=>$lrb['sisiya_admin.label.isactive']), 
		6=> array('key'=>'a.expire',	'label'=>$lrb['sisiya_admin.label.alert_frequency'])
	);
	$fields=array(
		...
		2 => array('field_name' => 'new_service_id',	'key' => 'a.serviceid',	 	'value' => '', 'is_str' => false),
		3 => array('field_name' => 'new_status_id',	'key' => 'a.statusid',	 	'value' => '', 'is_str' => false),
		4 => array('field_name' => 'new_alerttype_id',	'key' => 'a.alerttypeid',	'value' => '', 'is_str' => false),
		5 => array('field_name' => 'new_active',	'key' => 'a.enabled', 		'value' => '', 'is_str' => true),
		6 => array('field_name' => 'new_expire',	'key' => 'a.expire', 		'value' => '', 'is_str' => true)
	);
	...
	$table_header_fields=array(
		...
		1=> array('key'=>'i3.str',	'label'=>$lrb['sisiya_admin.services.label.keystr']), 
...
if(isset($_POST['add']))
	execSQLWrapper('add',"insert into ".$table_name." values(".$user_id.",".$_POST['new_system_id'].",".$_POST['new_service_id'].",".$_POST['new_alerttype_id'].",".$_POST['new_status_id'].",'".$_POST['new_active']."',".$_POST['new_expire'].",'".$alerttime."')");
	...
	for($i=0;$i<$nrows;$i++) {
		if(isset($_POST['delete'][$i])) {
			...
			execSQLWrapper('delete',"delete from ".$table_name." where userid=".$user_id." and systemid=".$_POST['system_id'][$i]." and serviceid=".$_POST['service_id'][$i]." and alerttypeid=".$_POST['alerttype_id'][$i]." and statusid=".$_POST['status_id'][$i]);
		}
		else if(isset($_POST['update'][$i])) {
			if($_SESSION['is_admin'] == 't')
				$user_id=$_POST['user_id'][$i];
			execSQLWrapper('update',"update ".$table_name." set expire=".$_POST['expire'][$i].",enabled='".$_POST['active'][$i]."' where userid=".$user_id." and systemid=".$_POST['system_id'][$i]." and serviceid=".$_POST['service_id'][$i]." and alerttypeid=".$_POST['alerttype_id'][$i]." and statusid=".$_POST['status_id'][$i]);
		}
	}
}
...
$sql_str="select a.systemid,a.alerttypeid,a.statusid,a.enabled,a.expire,b.hostname,i.str,i2.str,d.str,c.id,a.serviceid,i3.str";
if($_SESSION['is_admin'] == 't') 
	$sql_str.=",u.username,u.name,u.surname,a.userid";
$sql_str.=" from ".$table_name." a,";
$sql_str.="systems b,status c,systemtypes d,alerttypes e,services f,";
if($_SESSION['is_admin'] == 't') 
	$sql_str.="users u,";
$sql_str.="interface i,strkeys s,languages l,interface i2,strkeys s2,languages l2,interface i3,strkeys s3,languages l3";
$sql_str.=" where a.systemid=b.id and b.systemtypeid=d.id and a.statusid=c.id and a.alerttypeid=e.id and a.serviceid=f.id";
if($_SESSION['is_admin'] == 't') 
...
### service - language
$sql_str.=" and f.keystr=s3.keystr and s3.id=i3.strkeyid and i3.languageid=l3.id and l3.code=l.code ";
...
if(!$result)
	...
	if($nrows > 0) {
		...
		for($i=$start_index;$i<$start_index + $nrecords_per_page;$i++) {
			...
			if($_SESSION['is_admin'] == 't') {
				$html.="	<td>\n";
				$html.='	<input type="hidden" name="user_id['.$i.']" value="'.$row[15].'" />'."\n";	
				$html.=$row[13].' '.$row[14].' ('.$row[12].')'."\n";
				$html.="	</td>\n";
			}
			$html.="	<td>\n";
			...
			$html.='		<img height="25" src="'.getSystemTypeImage($row[8]).'" alt="'.$row[8].'" />&nbsp;'.$row[5];
			$html.="	</td>\n";
			$html.='	<td>'.$row[11]."</td>\n";
			$html.='	<td class="center"><img src="'.getStatusImage($row[2]).'" alt="'.$statusNames[$row[2]].'" /></td>'."\n";
			$html.='	<td>'.$row[7]."</td>\n";
