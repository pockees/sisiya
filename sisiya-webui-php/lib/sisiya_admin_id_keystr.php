...
if($_SESSION['is_admin'] == 'f' && $table_name == 'securitygroups')
	return;
...
if(isset($_POST['add']))
	execSQLWrapper('add',"insert into ".$table_name." values(".getNewID('new_id',$table_name).",'".$_POST['new_keystr']."')");
	...
	for($i=0;$i<$nrows;$i++) {
		if(isset($_POST['delete'][$i]))
			...
			execSQLWrapper('update','update '.$table_name." set keystr='".$_POST['keystr'][$i]."' where id=".$_POST['id'][$i]);
		}
}
...
$sql_str='select id,keystr from '.$table_name.$search_str.' order by '.$table_header_fields[$orderby_id]['key'].' '.$_SESSION['asc_desc'];
...
if(!$result)
	...
	if($nrows > 0) {
		...
		for($i=$start_index;$i<$start_index + $nrecords_per_page;$i++) {
			...
			$html.='	<td>'.getSelect('keystr['.$i.']',$row[1],$strkeys).'</td>'."\n";
			if($_SESSION['is_admin'] == 't') {
