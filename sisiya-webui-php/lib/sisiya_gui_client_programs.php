...
$navigation_panel_str.='<a href="'.$mainProg.'?menu=system_services'.$debug_str.'"><img src="'.$sisiyaImageDir.'/icon_system_services.png" alt="'.$lrb['sisiya_gui.system_services.header'].'" title="'.$lrb['sisiya_gui.label.system_services'].'" /></a>';

$h=$_SESSION['h'];
$h->addContent('<div>');
foreach($systems as $s => $ext) {
	$h->addContent('<table class="packages">');
	$h->addContent('<caption><img src="'.$imgDir.'/'.$s.'.gif" alt="'.$s.'" />'.$s.'</caption>');
	#h->addContent('<td>'.$lrb['sisiya_gui.label.client_packages'].'</td>');
	$h->addContent(getPackages($rootDir.'/packages/'.$s,$ext));
	$h->addContent('</table>');
}
$h->addContent('</div>');
?>
