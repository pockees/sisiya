...
$table_header_fields=array(
	...
	1 => array('key' => 'sortid',	'label' => $lrb['sisiya_admin.'.$table_name.'.label.sortid']),
	2 => array('key' => 'keystr',	'label' => $lrb['sisiya_admin.'.$table_name.'.label.keystr'])
);

$fields=array(
	0 => array('field_name' => 'new_id',		'key' => 'id', 		'value' => '', 'is_str' => false),
	1 => array('field_name' => 'new_sortid',	'key' => 'sortid', 	'value' => '', 'is_str' => false),
	2 => array('field_name' => 'new_keystr',	'key' => 'keystr', 	'value' => '', 'is_str' => true)
);
$nrows=get_setNRows($table_name);

if(isset($_POST['add']))
	execSQLWrapper('add',"insert into ".$table_name." values(".getNewID('new_id',$table_name).",".$_POST['new_sortid'].",'".$_POST['new_keystr']."')");
	...
	for($i=0;$i<$nrows;$i++) {
		if(isset($_POST['delete'][$i]))
			...
			execSQLWrapper('update','update '.$table_name." set sortid=".$_POST['sortid'][$i].",keystr='".$_POST['keystr'][$i]."' where id=".$_POST['id'][$i]);
		}
}
...
if($_SESSION['is_admin'] == 't') {
	...
	$html.='	<td><input class="id" type="text" name="new_sortid" value="'.getFieldValue($fields,'new_sortid').'" /></td>'."\n";
...
$sql_str='select id,sortid,keystr from '.$table_name.$search_str.' order by '.$table_header_fields[$orderby_id]['key'].' '.$_SESSION['asc_desc'];
...
if(!$result)
	...
	if($nrows > 0) {
		...
		for($i=$start_index;$i<$start_index + $nrecords_per_page;$i++) {
			...
			$html.='	<td><input class="id" type="text" name="sortid['.$i.']" value="'.$row[1].'" /></td>'."\n";
			$html.='	<td>'.getSelect('keystr['.$i.']',$row[2],$strkeys).'</td>'."\n";
			if($_SESSION['is_admin'] == 't') {
