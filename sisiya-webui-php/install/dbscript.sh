...
recreate_languages()
{
	sql_file=languages.sql
	echo "set FOREIGN_KEY_CHECKS=0;"	 > languages.sql
	echo "start transaction;"		>> languages.sql

	echo "delete from interface;"		>> languages.sql
	echo "delete from strkeys;"             >> languages.sql
	./language.sh                           >> languages.sql
	echo "set FOREIGN_KEY_CHECKS=1;"	>> languages.sql
	echo "commit;"				>> languages.sql
...
update_languages()
{
	sql_file="update_languages.sql"
	./update_languages.sh $conf_file > $sql_file
	$exec_sql_prog  $sql_file $dbuser $dbname $conf_file
	rm -f $sql_file
	return $?
}
