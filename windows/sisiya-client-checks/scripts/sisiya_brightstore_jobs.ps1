...
############################################################################################################
### service id
$serviceid=$serviceid_brightstore_jobs
if(! $serviceid_brightstore_jobs) {
	Write-Output "Error : serviceid_brightstore_jobs is not defined in the SisIYA client configuration file " $client_conf_file "!" | eventlog_error
	exit
}
############################################################################################################
### the default values
...
#ca_dbmgr -show jobs
$ca_qmgr_prog="C:\Program Files\CA\ARCserve Backup\ca_qmgr.exe"
