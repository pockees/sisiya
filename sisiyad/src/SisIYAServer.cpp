...
SisIYAServer::SisIYAServer(Connection * connection, int connsocket, int rt,
			   Semaphore * s, int log_level)
...
bool SisIYAServer::getNotOkMessage(int systemID, string & notOkMessage)
{
	...
	string sql =
	    string
	    ("select i.str,i2.str from systemservicestatus a,services b,status c,interface i,strkeys s,languages l,interface i2,strkeys s2,languages l2 where a.serviceid=b.id and a.statusid=c.id and systemid=")
	    + toString(systemID) +
	    string
	    (" and c.id > 2 and b.keystr=s.keystr and l.code='en' and l.id=i.languageid and i.strkeyid=s.id and c.keystr=s2.keystr and l2.code='en' and l2.id=i2.languageid and i2.strkeyid=s2.id order by statusid desc");
	...
	try {
		...
		syslog(LOG_ERR,
		       "getNotOkMessage: Error creating a Statement object! Caught SQLException : %s",
		       e.getMessage().c_str());
		return false;
	}
	ResultSet *rs = NULL;
	try {
		...
		syslog(LOG_ERR,
		       "getNotOkMessage: Caught SQLException! Reason: %s SQL State: %s Vendor error code: %d",
		       e.getMessage().c_str(), e.getSQLState().c_str(),
		       e.getErrorCode());
		delete rs;
		delete stmt;
	...
	if (rs == NULL) {
		syslog(LOG_ERR,
		       "getNotOkMessage: Query [%s] executiuon failed!",
		       sql.c_str());
		delete stmt;
		return false;
	} else if (loglevel > 2)
		syslog(LOG_INFO,
		       "getNotOkMessage: Query [%s] execution OK",
		       sql.c_str());
	ResultSetMetaData *rsmd;
	try {
		rsmd = rs->getResultSetMetaData();
		...
		syslog(LOG_ERR,
		       "getNotOkMessage: Caught SQLException! Could not create ResultSetMetadata object! Reason: %s SQL State: %s Vendor error code: %d",
		       e.getMessage().c_str(), e.getSQLState().c_str(),
		       e.getErrorCode());
		delete rs;
		delete stmt;
	...
	if (rsmd == NULL) {
		syslog(LOG_ERR,
		       "getNotOkMessage: Could not create ResultSetMetadata object!");
		delete rs;
		delete stmt;
		return false;
	}
	...
	while (rs->next()) {
		try {
			...
			syslog(LOG_ERR,
			       "getNotOkMessage: Caught SQLException! Could not get systemID Reason: %s SQL State: %s Vendor error code: %d",
			       e.getMessage().c_str(),
...
\return true on success or false on error.
*/
bool SisIYAServer::getSystemUpdateChangeTimes(int systemID,
					      string & updateTime,
					      string & changeTime)
{
	string sql =
...
*/
bool SisIYAServer::insertMessage(int systemID, int serviceID, int statusID,
				 long int expire,
				 const string & sendTimestamp,
				 const string & msg,
				 const string & msgdata)
{
	...
	string sql =
	    ...
	    string("','") + msg + string("','") + msgdata + string("')");
	...
	if (sID != INT_MIN) {
		...
		if (sID != statusID)
			sql =
			    ...
			    toString(expire) + string(",str='") + msg +
			    string("',data='") + msgdata +
		...
		sql =
		    ...
		    toString(expire) + string(",'") + msg + string("','") +
		    msgdata + string("')");
		try {
			if (stmt->executeUpdate(sql) != 1) {
				syslog(LOG_ERR,
...
bool SisIYAServer::extractMessageFields(const string m, int *serviceID,
...
bool SisIYAServer::processSystemMessages(const string & sendTimestamp,
					 const string & m)
{
	if (loglevel > 2)
		syslog(LOG_INFO,
		       "child server(%d) : processSystemMessages: length=%d Processing m=[%s]",
		       childPID, (int) m.length(), m.c_str());
	...
	if (loglevel > 2)
		syslog(LOG_INFO,
		       "child server(%d) : processSystemMessages : systemID=%d",
		       childPID, systemID);

	if (systemID <= 0) {
		if (systemID == 0)
			syslog(LOG_ERR,
			       "child server(%d) : no such system=%s",
			       childPID, systemName.c_str());
		else if (systemID < 2)
			syslog(LOG_ERR,
			       "child server(%d) : the system %s with systemID=%d is not enabled. Disable checks for this system or enable the system.",
			       childPID, systemName.c_str(),
			       -1 * systemID);
	...
	while (x.length() > 0) {
		...
		if (loglevel > 2)
			syslog(LOG_INFO,
			       "child server(%d) : processSystemMessages: length=%d Processing message_str=[%s]",
			       childPID, (int) message_str.length(),
			       message_str.c_str());
		if (message_str.length() == 0)
			break;	// there are no more messages
		if (extractMessageFields
		    (message_str, &serviceID, &statusID, &expire, msg,
		     data) == false) {
			syslog(LOG_ERR,
			       "child server(%d) : Bad formated message! %s",
			       childPID, message_str.c_str());
		...
		if (loglevel > 2)
			syslog(LOG_INFO,
			       "child server(%d) : processSystemMessages: serviceID=%d statusID=%d expire=%ld data=[%s]",
			       childPID, serviceID, statusID, expire,
			       data.c_str());
		// remove the processed part of the message
		x = x.substr(p + 10, x_length - p);	// </message> length is 10 => cut everything before end of the first </message> tag
		if (loglevel > 2)
			syslog(LOG_INFO,
			       "child server(%d) : processSystemMessages: length=%d x=[%s]",
			       childPID, (int) x.length(), x.c_str());
		...
		if (statusID < MESSAGE_INFO || statusID > MESSAGE_ALL) {
			syslog(LOG_ERR,
			       "child server(%d) : no such statusID=%d",
			       childPID, statusID);
		...
		if (checkServiceID(serviceID) == false) {
			syslog(LOG_ERR,
			       "child server(%d) : no such serviceID=%d",
			       childPID, serviceID);
		...
		if (insertMessage
		    (systemID, serviceID, statusID, expire, sendTimestamp,
		     msg, data) == false) {
			syslog(LOG_ERR,
			       "child server(%d) : Could not insert the message! %s",
			       childPID, message_str.c_str());
			continue;
		}
	...
	if (updateSystemStatus(systemID) == false) {
		syslog(LOG_ERR,
		       "child server(%d) : could not update systemstatus for systemID=%d",
		       childPID, systemID);
		return false;
	} else {
		if (loglevel > 2)
			syslog(LOG_INFO,
			       "child server(%d) : (1)updated systemstatus for systemID=%d",
			       childPID, systemID);
	}
	return true;
}
...
//              syslog(LOG_INFO,"child server(%d) : processSisIYAMessages: message=[%s]",childPID,m.c_str());
	...
	if (sisiya_messages.length() == 0) {
		syslog(LOG_ERR,
		       "child server(%d) : processSisIYAMessages: Malformed SisIYA message! Could not find any valid messages!",
		       childPID);
		return false;
	}
	// get timestamp
	string sendTimestamp =
	    extractXMLField(sisiya_messages, string("timestamp"));
	if (sendTimestamp.length() == 0) {
		syslog(LOG_ERR,
		       "child server(%d) : processSisIYAMessages: The length of send timestamp is 0!",
		       childPID);
		return false;
	}
	...
	if (loglevel > 2)
		syslog(LOG_INFO,
		       "child server(%d) : processSisIYAMessages: timestamp [%s]",
		       childPID, sendTimestamp.c_str());
	...
	while (x.length() > 0) {
		if (loglevel > 2)
			syslog(LOG_INFO,
			       "child server(%d) : processSisIYAMessages: length=%d Processing x=[%s]",
			       childPID, (int) x.length(), x.c_str());
		x_length = x.length();
		...
		if (system_str.length() == 0)
			break;
		if (processSystemMessages(sendTimestamp, system_str) ==
		    false) {
			syslog(LOG_ERR,
			       "child server(%d) : processSisIYAMessages: Error occured during processing of system messages! system_str=[%s]",
			       childPID, system_str.c_str());
...
bool SisIYAServer::process(void)
{
	...
	if (loglevel > 2) {
		if ((int) (clockticks = (double) sysconf(_SC_CLK_TCK)) ==
		    -1) {
			syslog(LOG_ERR,
			       "child server(%d) : could not get _SC_CLK_TCK!",
			       childPID);
			return false;
		}
		syslog(LOG_INFO,
		       "child server(%d) : the number of ticks per second is clockticks=%f",
		       childPID, clockticks);
		if ((int) clockticks == 0) {
			syslog(LOG_ERR,
			       "child server(%d) : the number of ticks per second is invalid!",
			       childPID);
			return false;
		}
		if ((int) (tcstart = times(&tmstart)) == -1) {
			syslog(LOG_ERR,
			       "child server(%d) : failed to get start time!",
			       childPID);
			return false;
		}
	}
	...
	if (esignal(SIGALRM, sig_alarm) == SIG_ERR) {
		syslog(LOG_ERR,
		       "child server(%d) : could not set signal handler for SIGALRM!",
		       childPID);
		return false;
	}
	...
	if (loglevel > 2)
		syslog(LOG_INFO,
		       "child server(%d) : starting to read from socket %d",
		       childPID, sfd);
	...
	while (1) {
		...
		if (sigsetjmp(env_alarm, 1) != 0) {
			if (loglevel > 2)
				syslog(LOG_INFO,
				       "child server(%d) : Read timeout=%d expired.",
				       childPID, readTimeout);
			syslog(LOG_INFO,
			       "child server(%d) : alarm (%d seconds) expired while reading from the socket=%d! ",
			       childPID, readTimeout, sfd);
			//clean up
			return false;
		...
		if (loglevel > 2)
			syslog(LOG_INFO,
			       "child server(%d) : canceled alarm (remaining seconds=%d) for ReadTimeout of %d seconds.",
			       childPID, remaining_time, readTimeout);
		...
		if (loglevel > 2)
			syslog(LOG_INFO,
			       "child server(%d) received data : [%s]\n",
			       childPID, line);
		if ((messages.max_size() - messages.length()) >=
		    strlen(line)) {
			if (loglevel > 2) {
				appendToLog(toString(number_of_reads) +
					    string(" --READ LINE--:") +
					    string(line));
				number_of_reads++;
			}
			...
			syslog(LOG_ERR,
			       "child server(%d) : Received message length exceeded max size of string, which is  %d! The rest of the message is discarded!",
			       childPID, (int) messages.max_size());
			break;
		}
	}
	if (loglevel > 2)
		syslog(LOG_INFO,
		       "child server(%d) : received messages : [%s]",
		       childPID, messages.c_str());


	if (loglevel > 2)
		syslog(LOG_INFO,
		       "child server(%d) : getting the lock from semaphore=%d",
		       childPID, sem->getID());
	sem->getLock();
	if (loglevel > 2)
		syslog(LOG_INFO,
		       "child server(%d) : got the lock from semaphore for %d",
		       childPID, sem->getID());
	// process received messages
	if (processSisIYAMessages(messages) == false) {
		syslog(LOG_ERR,
		       "child server(%d) : Error occured during processing of SisIYA messages!",
		       childPID);
	}
	if (loglevel > 2)
		syslog(LOG_INFO,
		       "child server(%d) : releasing the lock from semaphore id=%d",
		       childPID, sem->getID());
	sem->releaseLock();
	if (loglevel > 2)
		syslog(LOG_INFO,
		       "child server(%d) : released the lock from semaphore id=%d",
		       childPID, sem->getID());

	if (loglevel > 2) {
		if ((tcend = times(&tmend)) == -1) {
			syslog(LOG_ERR,
			       "child server(%d) : failed to get end time!",
			       childPID);
			return false;
		}
		cticks =
		    tmend.tms_utime + tmend.tms_stime - tmstart.tms_utime -
		    tmstart.tms_stime;
		syslog(LOG_INFO,
		       "child server(%d) : total CPU execution time = %f",
		       childPID, cticks / clockticks);
		if ((tcend <= tcstart) || tcend < 0 || tcstart < 0) {
			syslog(LOG_ERR,
			       "child server(%d) : tick time wrapped, couldn't calculate fraction!",
			       childPID);
			return false;
		}
		syslog(LOG_INFO,
		       "child server(%d) : fraction CPU time used is %f",
		       childPID, cticks / (tcend - tcstart));
	}
	return true;
}
...
bool SisIYAServer::updateSystemStatus(int systemID)
{
	...
	if (getSystemUpdateChangeTimes(systemID, updateTime, changeTime) ==
	    false) {
		syslog(LOG_INFO,
	...
	sql =
	    string
	    ("select cast(sum(statusid) as unsigned) from (select statusid from systemservicestatus where systemid=")
	    + toString(systemID) + string(" group by statusid) w");
	...
	if (systemStatusID != INT_MIN) {
		...
		if (totalStatusID < MESSAGE_WARNING) {	// The system is OK
			...
			sql =
			    ...
			    toString(mk) + string(",updatetime='") +
			    updateTime +
			    string("',str='System is OK' where systemid=")
			    + toString(systemID);
		} else {	// System has warnings and/or errors
			if (getNotOkMessage(systemID, notOkMessage) ==
			    false) {
				syslog(LOG_ERR,
				       "updateSystemStatus: Could not get error and/or warning messages for the system=%d. totalStatusID=%d",
				       systemID, totalStatusID);
			...
			if (totalStatusID != systemStatusID)
				sql =
				    ...
				    toString(totalStatusID) +
				    string(",changetime='") + changeTime +
				    string("',updatetime='") + updateTime +
				    string("',str='") + notOkMessage +
				    string("' where systemid=") +
				    toString(systemID);
			else
				sql =
