...
void Semaphore::create()
{
	...
	//if(key == static_cast<key_t>(IPC_PRIVATE)) { 
		...
		cerr << "Semaphore:create: Not intended for private semaphores." << endl;
		return;
	}
	//else if(key == (key_t)-1) {
	else if(key == static_cast<key_t>(-1)) {
		semID=-1;	// return(-1);	// probably an ftok() error by caller
		cerr << "Semaphore:create: Probably an ftok() error by caller." << endl;
		return;
	}
	again : 
		if((semID=semget(key,3,0666|IPC_CREAT)) < 0) {
			semID=-1;	// return(-1);	// permission problem or tables full
			cerr << "Semaphore:create: Permission problem or tables full." << endl;
			return;
		}
		/*
		...
		*/
		...
		int retcode;
		while((retcode=semop(semID,&op_lock[0],2)) < 0 && errno == EINTR) 
			; // try again
		if(retcode < 0) {
			if(errno == EINVAL)
				goto again;
...
*/
	...
	if(semID == -1) {
		cerr << "Semaphore::setKey: Could not create the semaphore with key=" << key << " initial=" << initial << endl;
		exit(1);
	}
}
...
void Semaphore::open(void)
{
	...
	//if(key == static_cast<key_t>(IPC_PRIVATE)) {
		...
		cerr << "Semaphore::open: Not intened for private semaphores." << endl;
		return;
	}
	//else if(key == (key_t) -1) {
	else if(key == static_cast<key_t>(-1)) {
		semID=-1;	//    return(-1);  // probably an ftok() error by caller
		cerr << "Semaphore::open: Probably an ftok() error by caller." << endl;
		return;
	}
	if((semID=semget(key,3,0)) < 0) {
		semID=-1;	// return(-1);  /* doesn't exist, or tables full
		cerr << "Semaphore::open: Does not exist or tables are full." << endl;
		return;
	}
	// Decrement the process counter. We don't need a lock to do this.
	//if(semop(semID,&op_open[0],1) < 0) {
	int retcode;
	while((retcode=semop(semID,&op_open[0],1)) < 0 && errno == EINTR)
		; // try again
	if(retcode < 0) {
		semID=-1;	//    err_sys("can't open");
		cerr << "Semaphore::open: Cannot open the semaphore." << endl;
		return;
	}
}
...
void Semaphore::remove(void)
{
	...
	if(semctl(semID,0,IPC_RMID,semctl_arg) < 0) {
		...
		switch(errno) {
			case EACCES :
				cerr << "The calling process has no access permissions needed to execute cmd.";
				break;
			case EFAULT :
				cerr << "The address pointed to by arg.buf or arg.array is not accessible";
				break;
			case EIDRM :
			...
			case EINVAL :
				...
				break;
			case EPERM :
				cerr << "The  argument  cmd has value IPC_SET or IPC_RMID but the calling process has";
				cerr << " insufficient privileges to execute the command.";
				break;
			case ERANGE :
				cerr << "The argument cmd has value SETALL or SETVAL and the value to which semval has to";
				cerr << " be  set  (for  some semaphore of the set) is less than 0 or greater than the ";
...
void Semaphore::close(void)
{
	...
	int retcode;
	while((retcode=semop(semID,&op_close[0],3)) < 0 && errno == EINTR) 
		; // try again
	if(retcode < 0) {
	...
	*/
	...
	if(semval > BIGCOUNT) {
		...
		while((retcode=semop(semID,&op_unlock[0],1)) < 0 && errno == EINTR) 
			; // try again
		if(retcode < 0) {
			// I should throw exception here.
...
void Semaphore::operation(int value)
{
	if((op_op[0].sem_op = value) == 0) {
		// I should throw exception here.
		cerr << "Semaphore::operation: Cannot have value=0 for semaphore operations." << endl;
		exit(1);
	}
	int retcode;
	while((retcode=semop(semID,&op_op[0],1) < 0) && errno == EINTR) 
		; // try again
	if(retcode < 0) {
		// I should throw exception here.
		cerr << "Semaphore::operation: semop error for semaphore=" << semID << " value=" << value << endl;
		exit(1);
	}
}
