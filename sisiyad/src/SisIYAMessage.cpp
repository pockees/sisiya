...
SisIYAMessage::SisIYAMessage(unsigned long int systemid,unsigned long int serviceid,unsigned short int statusid,unsigned long int expr,string stime,string rtime,string msg)
{
...
//! operator ==
int SisIYAMessage::operator==(const SisIYAMessage &m) const
{
	if(this->systemID != m.systemID) return 0;
	if(this->serviceID != m.systemID) return 0;
	if(this->statusID != m.systemID) return 0;
	if(this->expire != m.systemID) return 0;
	if(this->recieveTimestamp != m.recieveTimestamp) return 0;
	if(this->sendTimestamp != m.sendTimestamp) return 0;
	if(this->message != m.message) return 0;
	// otherwise
	return 1;
}
...
int SisIYAMessage::operator<(const SisIYAMessage &m) const
{
	...
	if(this->recieveTimestamp < m.recieveTimestamp) return 1;
	return 0;
}
