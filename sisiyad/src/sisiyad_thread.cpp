...
void doit(void);
bool extract_fields(char *msg,int *serviceID,int *statusID,long int *expire,string &hostName,string &str,string &sendTimestamp);
...
bool getErrorWarningMessage(Connection *conn,int systemID,string &error_warning);
...
bool insertMessage(Connection *conn,long unsigned int systemID,long unsigned int serviceID,short unsigned int statusID,long unsigned int expire,string sendTimestamp,string message);
bool isDone(void);
...
void showConfigs(void)
{
	...
	cout << "DBWriterThreads		: [" << confFile.getInt("DBWriterThreads") << "]" << endl;
	...
	cout << "DB_MAX_CONCURRENT_CONNECTIONS	: [" << confFile.getInt("DB_MAX_CONCURRENT_CONNECTIONS") << "]" << endl;
}
...
bool blockAllSignals(void)
{
	...
	if(sigfillset(&set) == -1) {
		syslog(LOG_ERR,"blockAllSignals: could not fill signal set!");
		return false;
	}
...
*/
	...
	if(pthread_create(&tid,NULL,controlManager,NULL) == -1) {
		syslog(LOG_ERR,"could not create the controlManager thread! Exiting...");
		clean_and_exit(EXIT_FAILURE);
	}

	// wait for the controManager thread	
	if(pthread_join(tid,NULL) != 0) {
		syslog(LOG_ERR,"doit:could not join the networkManager! Exiting...");
		clean_and_exit(EXIT_FAILURE);
	}
}
...
*/
	...
	while(1) {
		...
		if(!confFile.setFileName(confFileName)) {
			syslog(LOG_ERR,"controlManager: could open file %s",confFileName);
			clean_and_exit(EXIT_FAILURE);
		}
		if(loglevel > 1)
			syslog(LOG_INFO,"controlManager: set configuration parameters.");
		...
		if(pidFile.bad()) {
			syslog(LOG_ERR,"controlManager: can't open the PID file : %s ! Exiting...",confFile.getString("PID_FILE").c_str());
			clean_and_exit(EXIT_FAILURE);
		}
		pidFile << serverPID;
		pidFile.close();
		if(loglevel > 1)
			syslog(LOG_INFO,"controlManager: wrote server PID=%d to the PID file.",serverPID);

		// block all signals
		if(blockAllSignals() == false) {
			syslog(LOG_ERR,"controlManager: could not block all signals. Exiting...");
			clean_and_exit(EXIT_FAILURE);
		}
		if(loglevel > 1)
			syslog(LOG_INFO,"controlManager: blocked all signals.");
...
*/
			if(pthread_join(cacheManagerID,NULL) != 0) {
				syslog(LOG_ERR,"controlManager:could not join the cacheManager! Exiting...");
				clean_and_exit(EXIT_FAILURE);
			}
			if(loglevel > 1)
				syslog(LOG_INFO,"controlManager: joined the cacheManager thread.");
...
*/
			...
			for(i=0;i<nDBWriters;i++) {
				if(pthread_join(dbWriterIDs[i],NULL) != 0) {
					syslog(LOG_ERR,"controlManager:could not join the %d th dbWriter thread! Exiting...",i);
					clean_and_exit(EXIT_FAILURE);
				}
				else {
					if(loglevel > 1)
						syslog(LOG_INFO,"controlManager: joined the %d th dbWriter thread.",i);
				}
			}
		...
		//	delete[] dbWriterIDs;
			//dbWriterIDs=NULL;
			if(loglevel > 1)
				syslog(LOG_INFO,"controlManager: canceled %d dbWriter threads.",nDBWriters);
		}

		setDone(false);
		// create the networkManager thread
		if(pthread_create(&networkManagerID,NULL,networkManager,NULL) == -1) {
			syslog(LOG_ERR,"controlManager: could not create the networkManager thread! Exiting...");
			clean_and_exit(EXIT_FAILURE);
		}
		if(loglevel > 1)
			syslog(LOG_INFO,"controlManager: created the networkManager thread.");


		// create the cacheManager thread
		if(pthread_create(&cacheManagerID,NULL,cacheManager,NULL) == -1) {
			syslog(LOG_ERR,"controlManager: could not create the cacheManager thread! Exiting...");
			clean_and_exit(EXIT_FAILURE);
		}
		if(loglevel > 1)
			syslog(LOG_INFO,"controlManager: created the cacheManager thread.");
		...
		if(nDBWriters < 1) {
			...
			syslog(LOG_ERR,"controlManager: DBWriterThreads=%d < 1! There is no way at the moment to work without dbWriter threads!",nDBWriters);
		...
		for(i=0;i<nDBWriters;i++) {
			...
			if(pthread_create(&dbWriterIDs[i],NULL,dbWriter,NULL) == -1) {
				syslog(LOG_ERR,"controlManager: could not create the %d th dbWriter thread! Exiting...",i);
				clean_and_exit(EXIT_FAILURE);
			}
			else {
				if(loglevel > 1)
					syslog(LOG_INFO,"controlManager: created the %d th dbWriter thread.",i);
			}
		}
		if(loglevel > 1)
			syslog(LOG_INFO,"controlManager: created all %d dbWriter threads.",nDBWriters);


		if(sigemptyset(&set) == -1) {
			syslog(LOG_ERR,"controlManager: could not empty signal set!");
			clean_and_exit(EXIT_FAILURE);
		}
		if(sigaddset(&set,SIGHUP) == -1) {
			syslog(LOG_ERR,"controlManager: could not add SIGHUP to the signal set!");
			clean_and_exit(EXIT_FAILURE);
		}
		sigwait(&set,&signum);
		if(loglevel > 1)
			syslog(LOG_INFO,"controlManager: caught SIGHUP signal. Reconfiguring the SisIYA daemon ...");
		setDone(true);
	}
}
...
void setDone(bool value)
{
	...
	if(lockMutex(doneMutex) == false) {
		syslog(LOG_ERR,"setDone: could not lock the mutex doneMutex!");
		return;
	}
	done=value;
	if(unlockMutex(doneMutex) == false) {
		syslog(LOG_ERR,"isDone: could not unlock the mutex doneMutex!");
		return; 
	}
}
...
bool isDone(void)
{
	...
	if(lockMutex(doneMutex) == false) {
		syslog(LOG_ERR,"isDone: could not lock the mutex doneMutex!");
...
void *networkManager(void *arg)
{
	...
	if((listenfd=socket(AF_INET,SOCK_STREAM,0)) < 0) {
		syslog(LOG_ERR,"networkManager: can't open stream socket (socket(AF_INET,SOCK_STREAM,0))");
		exit(1);
	}
 
	if(loglevel > 1)
		syslog(LOG_INFO,"networkManager: opened a stream socket.");
	...
	if(confFile.getString("IP") == "any" || confFile.getString("IP") == "0.0.0.0")
		...
		if(inet_aton(confFile.getString("IP").c_str(),&(serv_addr.sin_addr)) == 0) {
			syslog(LOG_ERR,"networkManager: Can't convert numbers-and-dots (%s) notation into  binary  data",confFile.getString("IP").c_str());
			exit(1);
		} 
	} 
	if(loglevel > 2)
		syslog(LOG_INFO,"networkManager: inet_ntoa(serv_addr.sin_addr)=%s",inet_ntoa(serv_addr.sin_addr));
	...
	if(setsockopt(listenfd,SOL_SOCKET,SO_REUSEADDR, &on, sizeof(on)) < 0) {
		close(listenfd);
		syslog(LOG_ERR,"networkManager: Cannot set SO_REUSEADDR socket option! Exiting...");
		exit(1);
	}
	else {
		if(loglevel > 2)
			syslog(LOG_INFO,"networkManager: Set SO_REUSEADDR socket option");
	}
 
	if(bind(listenfd,(struct sockaddr *)&serv_addr,sizeof(serv_addr)) < 0) {
//	if(bind(listenfd,static_cast<struct sockaddr *>(&serv_addr),sizeof(serv_addr)) < 0) {
		close(listenfd);
		syslog(LOG_ERR,"networkManager: Cannot bind local address (%s:%d). Errno=%d",confFile.getString("IP").c_str(),confFile.getInt("PORT"),errno);
		exit(1);
	} 

	if(loglevel > 1)
		syslog(LOG_INFO,"networkManager: Bind local address (%s:%d)",confFile.getString("IP").c_str(),confFile.getInt("PORT"));

	if(listen(listenfd,LISTENQ) == -1) {
		syslog(LOG_ERR,"networkManager: Cannot listen address (%s:%d)",confFile.getString("IP").c_str(),confFile.getInt("PORT"));
		exit(1);
	}
	...
	syslog(LOG_INFO,"networkManager: Server (version=%s) started and accepting connection on interface=%s and port=%d. PID is %d",VERSION,confFile.getString("IP").c_str(),confFile.getInt("PORT"),serverPID); 
	...
	for(;;) {
		...
		if(nready == -1) {
			syslog(LOG_ERR,"networkManager: error calling poll! Error code=%d message=%s",errno,strerror(errno));
			clean_and_exit(errno);
		}
		if(nready == 0) {
			if(loglevel > 1)
				syslog(LOG_INFO,"networkManager: poll timed out.");
			// test if we should exit
			if(isDone()) {
				syslog(LOG_INFO,"networkManager: done is set. Exiting...");
				close(listenfd);
				return NULL;
			}
			continue;
		...
		if(!(plisten[0].revents & POLLRDNORM)) {
			syslog(LOG_INFO,"networkManager: not a new connection!?");
			continue;
		}
		addrlen=static_cast<socklen_t>(sizeof(cli_addr));
		connfd=(int *)malloc(sizeof(int));
		if(connfd == NULL) {
			syslog(LOG_ERR,"networkManager: could not allocate memory for connfd (int)! Error code=%d message=%s",errno,strerror(errno));
			clean_and_exit(errno);
		}
		*connfd=accept(listenfd,(struct sockaddr *)&cli_addr,&addrlen);

		if((*connfd) == -1) {
			syslog(LOG_ERR,"networkManager: error calling accept! Error code=%d message=%s",errno,strerror(errno));
			clean_and_exit(errno);
		}
		if(loglevel > 1)
			syslog(LOG_INFO,"networkManager: client(%s:%d) connected.",inet_ntoa(cli_addr.sin_addr),cli_addr.sin_port);
...
Connection *getConnection(void)
{
	...
	string edbc=string("edbc:")+lowerCase(confFile.getString("DB_TYPE"))+string("://")+confFile.getString("DB_SERVER")+string("/")+confFile.getString("DB_NAME");
	if(confFile.getInt("DB_PORT") != 0)
		edbc+=string(":")+confFile.getString("DB_PORT");
	if(dm == NULL) {
		syslog(LOG_ERR,"getConnection: Driver manager is not available!");
		return NULL;
	}
	try {
		...
		syslog(LOG_ERR,"getConnection: Failed to connect to DB %s@%s",confFile.getString("DB_USER").c_str(),edbc.c_str());
		//exit(1);
		return NULL;
	}
	if(conn == NULL) {
		syslog(LOG_ERR,"getConnection: Could not connect to connect DB %s@%s",confFile.getString("DB_USER").c_str(),edbc.c_str());
		//clean_and_exit(1);
		return NULL;
	}
	else
		if(loglevel > 1)
			syslog(LOG_INFO,"getConnection: Connected to connect DB %s@%s",confFile.getString("DB_USER").c_str(),edbc.c_str());
...
bool extract_fields(char *msg,int *serviceID,int *statusID,long *expire,string &hostName,string &str,string &sendTimestamp)
{
	char sp[2];
...
//	syslog(LOG_INFO,"extract_fields: line=[%s]",msg);
	...
	if(j == 6) {
		...
		isstr >> (*serviceID) >> (*statusID) >> hostName >> sendTimestamp >> (*expire);
		if(loglevel > 2)
			syslog(LOG_INFO,"child server: extract_fields: hostName=[%s] serviceID=%d statusID=%d sendTimestamp=[%s] expire=%ld",hostName.c_str(),*serviceID,*statusID,sendTimestamp.c_str(),*expire);
		// check the sendTimestamp value
		if(sendTimestamp.size() != 14) {
			syslog(LOG_ERR,"child server: extract_fields: sendTimestamp=[%s] has length of %d != 14!",sendTimestamp.c_str(),sendTimestamp.size());
			return false;
		}
		// all chars must be at lest digits (0-9)
		for(int k=0;k<(int)sendTimestamp.size();k++) {
			if(isdigit(sendTimestamp[k]) == 0) {
				syslog(LOG_ERR,"child server: extract_fields: sendTimestamp=[%s] the %d th char is=%c which is not a digit!",sendTimestamp.c_str(),k+1,sendTimestamp[k]);
				return false;
			}
		}	
		// now check the date time itself,
		//int year=fromString<int>(string(sendTimestamp.substr(0,4)));
		int month=fromString<int>(string(sendTimestamp.substr(4,2)));
		int day=fromString<int>(string(sendTimestamp.substr(6,2)));
		int hour=fromString<int>(string(sendTimestamp.substr(8,2)));
		int minutes=fromString<int>(string(sendTimestamp.substr(10,2)));
		int seconds=fromString<int>(string(sendTimestamp.substr(12,2)));
...
*/
		if(month > 12) {
			syslog(LOG_ERR,"child server: extract_fields: sendTimestamp=[%s] invalid month=%d (> 12)!",sendTimestamp.c_str(),month);
			return false;
		}
		if(day > 31) {
			syslog(LOG_ERR,"child server: extract_fields: sendTimestamp=[%s] invalid day=%d (> 31)!",sendTimestamp.c_str(),day);
			return false;
		}
		if(hour >23) {
			syslog(LOG_ERR,"child server: extract_fields: sendTimestamp=[%s] invalid hour=%d (> 23)!",sendTimestamp.c_str(),hour);
			return false;
		}
		if(minutes > 59) {
			syslog(LOG_ERR,"child server: extract_fields: sendTimestamp=[%s] invalid minutes=%d (> 59)!",sendTimestamp.c_str(),minutes);
			return false;
		}
		if(seconds > 59) {
			syslog(LOG_ERR,"child server: extract_fields: sendTimestamp=[%s] invalid seconds=%d (> 59)!",sendTimestamp.c_str(),seconds);
			return false;
		}
	}
	else
...
int getSystemIDFromCache(string hostName)
{
	...
	if(conn == NULL) {
		syslog(LOG_INFO,"getSystemIDFromCache: could not get connection object!");
		return systemID;
	}

	systemID=getSystemID(conn,hostName);
	if(loglevel > 2)
		syslog(LOG_INFO,"getSystemIDFromCache: systemID=%d",systemID);

	if(systemID <= 0) { 
		if(systemID == 0)
			syslog(LOG_ERR,"getSystemIDFromCache: no such system=%s",hostName.c_str());
		else if(systemID < 2)
			syslog(LOG_ERR,"getSystemIDFromCache: the system %s with systemID=%d is not enabled. Disable client check for this or enabled the system.",hostName.c_str(),-1*systemID);
	}
	delete conn;
	return systemID;
}	
...
bool getErrorWarningMessage(Connection *conn,int systemID,string &error_warning)
{
	...
	try {
		...
		syslog(LOG_ERR,"getErrorWarningMessage: Error creating a Statement object! Caught SQLException : %s",e.getMessage().c_str());
		return false;
	}
	ResultSet *rs=NULL;
	try {
		...
		syslog(LOG_ERR,"getErrorWarningMessage: Caught SQLException! Reason: %s SQL State: %s Vendor error code: %d",e.getMessage().c_str(),e.getSQLState().c_str(),e.getErrorCode());
		delete rs;
		delete stmt;
		return false;
	}
	if(rs == NULL) {
		syslog(LOG_ERR,"getErrorWarningMessage: Query [%s] executiuon failed!",sql.c_str());
		delete stmt;
		return false;
	}
	else
		if(loglevel > 2)
			syslog(LOG_INFO,"getErrorWarningMessage: Query [%s] execution OK",sql.c_str());
	ResultSetMetaData *rsmd;
	try {
		rsmd=rs->getResultSetMetaData();
	}
	catch(SQLException &e) {
		syslog(LOG_ERR,"getErrorWarningMessage: Caught SQLException! Could not create ResultSetMetadata object! Reason: %s SQL State: %s Vendor error code: %d",e.getMessage().c_str(),e.getSQLState().c_str(),e.getErrorCode());
		delete rs;
		delete stmt;
		return false;
	}
	if(rsmd == NULL) {
		syslog(LOG_ERR,"getErrorWarningMessage: Could not create ResultSetMetadata object!");
		delete rs;
		delete stmt;
		return false;
	}
	...
	while(rs->next()) {
		try {
			...
			syslog(LOG_ERR,"getErrorWarningMessage: Caught SQLException! Could not get systemID Reason: %s SQL State: %s Vendor error code: %d",e.getMessage().c_str(),e.getSQLState().c_str(),e.getErrorCode());
			delete rsmd;
			delete rs;
			delete stmt;
			return false;
...
bool updateAllSystemStatus(void)
{
	Connection *conn=getConnection();
	if(conn == NULL) {
		syslog(LOG_ERR,"updateAllSystemStatus: could not get Connection object");
		return false;
	}
	...
	try {
		...
		syslog(LOG_ERR,"updateAllSystemStatus: Error creating a Statement object! Caught SQLException : %s",e.getMessage().c_str());
		delete conn;
		return false;
	}
	ResultSet *rs=NULL;
	try {
		...
		syslog(LOG_ERR,"updateAllSystemStatus: Caught SQLException! Reason: %s SQL State: %s Vendor error code: %d",e.getMessage().c_str(),e.getSQLState().c_str(),e.getErrorCode());
		delete rs;
		delete stmt;
		delete conn;
		return false;
	...
	if(rs == NULL) {
		syslog(LOG_ERR,"updateAllSystemStatus: Query [%s] executiuon failed!",sql.c_str());
		delete stmt;
		return false;
	}
	else
		if(loglevel > 3)
			syslog(LOG_INFO,"updateAllSystemStatus: Query [%s] execution OK",sql.c_str());
	ResultSetMetaData *rsmd;
	try {
		rsmd=rs->getResultSetMetaData();
	}
	catch(SQLException &e) {
		syslog(LOG_ERR,"updateAllSystemStatus: Caught SQLException! Could not create ResultSetMetadata object! Reason: %s SQL State: %s Vendor error code: %d",e.getMessage().c_str(),e.getSQLState().c_str(),e.getErrorCode());
		delete rs;
		delete stmt;
		delete conn;
		return false;
	...
	if(rsmd == NULL) {
		syslog(LOG_ERR,"updateAllSystemStatus: Could not create ResultSetMetadata object!");
		delete rs;
		delete stmt;
		delete conn;
		return false;
	...
	while(rs->next()) {
		try {
			if(updateSystemStatus(conn,rs->getInt(0)) == false) {
				syslog(LOG_ERR,"updateAllSystemStatus: Could not create ResultSetMetadata object!");
				delete rsmd;
				delete rs;
				delete stmt;
...
bool updateSystemStatus(Connection *conn,int systemID)
{
	...
	if(loglevel > 2)
		syslog(LOG_INFO,"updateSystemStatus: Got system max(statusid)=%d for systemID=%d",maxStatusID,systemID); 

	if(systemStatusID != INT_MIN) {
		// it is going to be an update
		if(maxStatusID < 2) 	// The system is OK
			sql=string("update systemstatus set statusid=")+toString(MESSAGE_OK)+string(",updatetime='")+updateTime+string("',str='System is OK' where systemid=")+toString(systemID);
		else {	// System has warnings and/or errors
			if(getErrorWarningMessage(conn,systemID,error_warning) == false) {
				syslog(LOG_ERR,"updateSystemStatus: Could not get error and/or warning messages for the system=%d. maxStatusID=%d",systemID,maxStatusID);
				error_warning="There was no status info!";
			}
			if(maxStatusID != systemStatusID)
				sql=string("update systemstatus set statusid=")+toString(maxStatusID)+string(",changetime='")+changeTime+string("',updatetime='")+updateTime+string("',str='")+error_warning+string("' where systemid=")+toString(systemID);
			else
				sql=string("update systemstatus set updatetime='")+updateTime+string("',str='")+error_warning+string("' where systemid=")+toString(systemID);
		}	  
	}
	else {
		...
		if(systemStatusID < 2)
			...
			sql=string("insert into systemstatus values(")+toString(systemID)+string(",")+toString(systemStatusID)+string(",'")+updateTime+string("','")+changeTime+string("','")+error_warning+string("')");
		}
	}
...
bool insertMessage(Connection *conn,SisIYAMessage &sm)
{
	return(insertMessage(conn,sm.getSystemID(),sm.getServiceID(),sm.getStatusID(),sm.getExpire(),sm.getSendTimestamp(),sm.getMessage()));
}
...
bool insertMessage(Connection *conn,long unsigned int systemID,long unsigned int serviceID,short unsigned int statusID,long unsigned int expire,string sendTimestamp,string message)
{
	...
	string sql=string("insert into systemhistorystatus values('")+sendTimestamp+string("',")+toString(systemID)+string(",")+toString(serviceID)+string(",")+toString(statusID)+string(",'")+recieveTimestamp+string("','")+message+string("')");
	...
	if(sID != INT_MIN) {
		// update 
		if(sID != statusID) 
			sql=string("update systemservicestatus set statusid=")+toString(statusID)+string(",changetime='")+recieveTimestamp+string("',updatetime='")+recieveTimestamp+string("',expires=")+toString(expire)+string(",str='")+message+string("' where systemid=")+toString(systemID)+string(" and serviceid=")+toString(serviceID);
		else 
			sql=string("update systemservicestatus set statusid=")+toString(statusID)+string(",updatetime='")+recieveTimestamp+string("',expires=")+toString(expire)+string(",str='")+message+string("' where systemid=")+toString(systemID)+string(" and serviceid=")+toString(serviceID);
		try {
			if(stmt->executeUpdate(sql) != 1) {
		...
		// new record
		sql=string("insert into systemservicestatus values(")+toString(systemID)+string(",")+toString(serviceID)+string(",")+toString(statusID)+string(",'")+recieveTimestamp+string("','")+recieveTimestamp+string("',")+toString(expire)+string(",'")+message+string("')");
		try {
			if(stmt->executeUpdate(sql) != 1) {
				syslog(LOG_ERR,"insertMessage: Query [%s] executiuon failed! Insert must produce exactly 1 row?!",sql.c_str());
				delete stmt;
...
*/
	...
	Connection *conn=getConnection();
	if(conn == NULL) {
		syslog(LOG_ERR,"cacheManager: could not connect to the database!");
		//return false;
		clean_and_exit(1);
	}
	while(1) {
		if(lockMutex(cacheMutex) == false) {
			syslog(LOG_ERR,"cacheManager: could not lock the mutex cacheMutex!");
			return false;
		}
		if(loglevel > 3)
			syslog(LOG_INFO,"cacheManager: get lock on the cachMutex");
		...
		if(updateCache(conn) == false) {
			syslog(LOG_ERR,"cacheManager: could not update the cache!");
		}
		else {
			if(loglevel > 3)
				syslog(LOG_INFO,"cacheManager: updated the cache.");
		}
		if(updateDB(conn) == false) {
			syslog(LOG_ERR,"cacheManager: could not update DB from cache!");
		}
		else {
			if(loglevel > 3)
				syslog(LOG_INFO,"cacheManager: updated DB from cache.");
		}
		if(unlockMutex(cacheMutex) == false) {
			syslog(LOG_ERR,"cacheManager: could not unlock the mutex cacheMutex!");
			return false;
		}
		if(loglevel > 3)
			syslog(LOG_INFO,"cacheManager: unlocked the mutex cacheMutex after updating the cache");
		if(isDone()) {
			syslog(LOG_INFO,"cacheManager: done is set. Exiting...");
			break;
		}
		// sleep or cond wait
		//sleep(180); // 3 minutes
...
bool createSignalManager(void)
{
	...
	if(retcode != 0) {
		syslog(LOG_ERR,"createSignalManager: could not initialize default thread attribute! Error code=%d",retcode);
		return false;		
	}
	retcode=pthread_attr_getschedparam(&threadAttr,&param);
	if(retcode != 0) {
		syslog(LOG_ERR,"createSignalManager: could not get schedule parameters! Error code=%d",retcode);
		return false;		
	}
	retcode=pthread_attr_getschedpolicy(&threadAttr,&policy);
	if(retcode != 0) {
		syslog(LOG_ERR,"createSignalManager: could not get schedule policy! Error code=%d",retcode);
		return false;		
	}
	if(param.sched_priority < sched_get_priority_max(policy)) {
		...
		if(retcode != 0) {
			syslog(LOG_ERR,"createSignalManager: could not set schedule parameter! Error code=%d",retcode);
			return false;		
		}
	}
	pthread_t tid;
	...
	if(retcode != 0) {
		syslog(LOG_ERR,"createSignalManager: could not get create signalManager thread! Error code=%d",retcode);
		return false;		
	}
...
void *signalManager(void *arg)
{
	...
	if(retcode == -1) {
		syslog(LOG_ERR,"signalManager: could not empty signal set!");
		clean_and_exit(EXIT_FAILURE);
	}
	retcode=sigaddset(&set,SIGTERM);
	if(retcode == -1) {
		syslog(LOG_ERR,"signalManager: could not add SIGTERM to the signal set!");
		clean_and_exit(EXIT_FAILURE);
	}
	retcode=sigaddset(&set,SIGQUIT);
	if(retcode == -1) {
		syslog(LOG_ERR,"signalManager: could not add SIGQUIT to the signal set!");
		clean_and_exit(EXIT_FAILURE);
	}
	retcode=sigaddset(&set,SIGUSR1);
	if(retcode == -1) {
		syslog(LOG_ERR,"signalManager: could not add SIGUSR1 to the signal set!");
		clean_and_exit(EXIT_FAILURE);
	}
/*
...
*/
	while(1) {
		...
		switch(signum) {
			case SIGQUIT :
				syslog(LOG_INFO,"signalManager: caught SIGQUIT signal. Exiting...");
				clean_and_exit(EXIT_SUCCESS);
				break;
			case SIGTERM :
				syslog(LOG_INFO,"signalManager: caught SIGTERM signal. Exiting ...");
				clean_and_exit(EXIT_SUCCESS);
				break;
			case SIGUSR1 :
				syslog(LOG_INFO,"signalManager: caught SIGUSR1 signal. Printing SisIYA configuration to syslog ...");
				sisiya_showconf();
				break;
			default :
				syslog(LOG_INFO,"signalManager: caught unexpected signal=%d.",signum);
				break;
		}
	}
	return NULL;
...
bool detachThread(pthread_t *tid)
{
	...
	switch(retcode) {
		case 0 : // OK
			break;
		case ESRCH :
			syslog(LOG_ERR,"no thread could be found to detach! Etrror code=%d message=%s",retcode,strerror(retcode));
			return false;
			break;
		case EINVAL :
			syslog(LOG_ERR,"thread is already in detached state! Etrror code=%d message=%s",retcode,strerror(retcode));
			return false;
			break;
		default :
			syslog(LOG_ERR,"could not detach thread! Etrror code=%d message=%s",retcode,strerror(retcode));
			return false;
			break;
	}
	return true;
...
*/
	...
	Connection *conn=getConnection();
	if(conn == NULL) {
		syslog(LOG_ERR,"dbWriter: could not connect to the database!");
		//return false;
		clean_and_exit(1);
	}
	while(1) {
		...
		if(lockMutex(mqueueMutex) == false) {
			syslog(LOG_ERR,"dbWriter: could not lock the mutex mqueueMutex!");
			return false;
		}
		if(loglevel > 3)
			syslog(LOG_INFO,"dbWriter: got lock on the mqueueMutex");

		if(mqueue.size() < 1) {
			if(loglevel > 3)
				syslog(LOG_INFO,"dbWriter: waiting for condition on mqueueCond and mqueueMutex.");
			// go to sleep
			if(condWait(mqueueCond,mqueueMutex) == false) {
				syslog(LOG_ERR,"dbWriter: could wait for condition on mqueueCond and mutex mqueueMutex!");
				return false;
			}
			else {
				if(loglevel > 3)
					syslog(LOG_INFO,"dbWriter: wake up after waiting for condition on mqueueCond and mqueueMutex.");
				if(isDone()) {
					syslog(LOG_INFO,"dbWriter: done is set. Exiting...");
					break;
				}
			}
		}
		if(mqueue.size() < 1) {
			if(unlockMutex(mqueueMutex) == false) {
				syslog(LOG_ERR,"dbWriter: could not unlock the mutex mqueueMutex!");
				return false;
			}
			else {
				if(loglevel > 3)
					syslog(LOG_INFO,"dbWriter: unlocked the mqueueMutex. Nothing to do.");
			}
			continue;
		}
		...
		if(loglevel > 3)
			syslog(LOG_INFO,"dbWriter: unlocked the mutex mqueueMutex after adding to the queue");
...
//syslog(LOG_INFO,"dbWriter: systemid=%ld serviceid=%ld statusid=%d sendTimestamp=[%s] expires=[%ld] msg=[%s]",sm.getSystemID(),sm.getServiceID(),sm.getStatusID(),sm.getSendTimestamp().c_str(),sm.getExpire(),sm.getMessage().c_str());
		...
		if(insertMessage(conn,sm) == false) {
			syslog(LOG_ERR,"dbWriter: could not insert the message %s!",sm.getMessage().c_str());
		}
...
bool processClient(int connfd)
{
	...
	if(loglevel > 2) 
		syslog(LOG_INFO,"processClient: starting to read from socket %d",connfd);
	while(1) {
		if((n=readline(connfd,line,MAX_STR)) < 0) {
			syslog(LOG_ERR,"processClient: socket read error");
			return false; 
		}
		if(n == 0)
			break;
		...
		if(loglevel > 2) 
			syslog(LOG_INFO,"processClient: recieved data : [%s]\n",line);
		if(extract_fields(line,&serviceID,&statusID,&expire,hostName,str,sendTimestamp) == false) {
			syslog(LOG_ERR,"processClient: msg=[%s] has wrong format!",line);
			continue;
		}
		if(loglevel > 2)
			syslog(LOG_INFO,"processClient: serviceid=%d statusid=%d hostname=[%s] sendTimestamp=[%s] expires=[%ld] msg=[%s]",serviceID,statusID,hostName.c_str(),sendTimestamp.c_str(),expire,str.c_str());
		...
		string recieveTimestamp;
		getTimestamp(recieveTimestamp);
		SisIYAMessage sm((unsigned long int)systemID,(unsigned long int)serviceID,(unsigned short int)statusID,(unsigned long int)expire,sendTimestamp,recieveTimestamp,str);

		if(loglevel > 2)
			syslog(LOG_INFO,"processClient: systemid=%ld serviceid=%ld statusid=%d hostname=[%s] sendTimestamp=[%s] expires=[%ld] msg=[%s]",sm.getSystemID(),sm.getServiceID(),sm.getStatusID(),hostName.c_str(),sm.getSendTimestamp().c_str(),sm.getExpire(),sm.getMessage().c_str());

		// lock
		if(lockMutex(mqueueMutex) == false) {
			syslog(LOG_ERR,"processClient: could not lock the mutex mqueueMutex!");
			return false;
		}
		if(loglevel > 3)
			syslog(LOG_INFO,"processClient: get lock on the mqueueMutex");

		// add the message to the end of the queue
		mqueue.push_back(sm);
	
		if(unlockMutex(mqueueMutex) == false) {
			syslog(LOG_ERR,"processClient: could not unlock the mutex mqueueMutex!");
			return false;
		}
		if(loglevel > 3)
			syslog(LOG_INFO,"processClient: unlocked the mutex mqueueMutex after adding to the queue");
	}
	if(loglevel > 2)
		syslog(LOG_ERR,"processClient: queue size=%d",mqueue.size());
	return true;
}
...
bool lockMutex(pthread_mutex_t &mutex)
{
	...
	switch(retcode) {
		case 0 :
			...
			//break;
		case EINVAL :
			syslog(LOG_ERR,"lockMutex: child server : the mutex is not properly inituialized!");
			break;
		case EDEADLK :
			syslog(LOG_ERR,"lockMutex: child server : the mutex is already lock by the calling thread!");
			break;
		default :
			syslog(LOG_ERR,"lockMutex: child server : got unknown error code=%d while trying to lock the mutex!",retcode);
			break;
	}
	return false;
}
...
bool unlockMutex(pthread_mutex_t &mutex)
{
	...
	switch(retcode) {
		case 0 :
			...
			//break;
		case EINVAL :
			syslog(LOG_ERR,"unlockMutex: child server : the mutex is not properly inituialized");
			break;
		case EPERM :
			syslog(LOG_ERR,"unlockMutex: child server : the calling thread does not own this mutex!");
			break;
		default :
			syslog(LOG_ERR,"unlockMutex: child server : got unknown error code=%d while trying to unlock the mutex!",retcode);
			break;
	}
	return false;
}
...
bool condWait(pthread_cond_t &cond,pthread_mutex_t &mutex)
{
	...
	syslog(LOG_ERR,"condWait: child server : got unknown error while waiting on the conditional variable!");
	return false;
...
bool condSignal(pthread_cond_t &cond)
{
	if(pthread_cond_signal(&cond) == 0)
		return true;
	// According to the man page this function never returns a error code.
	syslog(LOG_ERR,"condSignal: child server : got unknown error while signalling for the conditional variable!");
	return false;
}
...
bool condBroadcast(pthread_cond_t &cond)
{
	if(pthread_cond_broadcast(&cond) == 0)
		return true;
	// According to the man page this function never returns a error code.
	syslog(LOG_ERR,"condBroadcast: child server : got unknown error while broadcasting the conditional variable!");
	return false;
