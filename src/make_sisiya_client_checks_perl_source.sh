...
cp -a ${sisiya_dir}/sisiya_check_perl/* $package_dir
echo "${version_str}-$release_str" > $package_dir/version.txt

mkdir -p $package_dir/etc/cron.d
cp ${sisiya_dir}/etc/cron.d/sisiya_client_checks_perl $package_dir/etc/cron.d/sisiya_client_checks
if test $# -eq 1 ; then
	...
	if test -f ${local_dir}/sisiya-client-checks/SisIYA_Config_local.pl ; then
		echo "I am using your own SisIYA_Config_local.pl file (${local_dir}/sisiya-client-checks/SisIYA_Config_local.pl) ..."
		cp -f ${local_dir}/sisiya-client-checks/SisIYA_Config_local.pl $package_dir/
