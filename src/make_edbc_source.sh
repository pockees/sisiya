...
cp -a ${sisiya_dir}/edbc/* ${package_str}${version}
...
for f in Doxyfile examples include lib Makefile mysql postgresql src utils 
do
...
if test ! -d $edbc_dir ; then
	echo "Directory $edbc_dir does not exist. Exiting..."
	exit 1
fi
...
rm -rf edbc${version}
mkdir -p edbc${version}
cp -a ${edbc_dir}/* edbc${version}
### now make the source package
tar cfz edbc${version}.tar.gz edbc${version}
rm -rf edbc${version}
###
echo "In order to build the EDBC packages one can use the following command:"
echo "rpmbuild -ta edbc${version}.tar.gz"
