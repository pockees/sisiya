...
static struct sembuf op_lock[2]=
    {
     ...
     {2,1,SEM_UNDO}   /* then increment [2] to 1 - this locks it         */
                    /* UNDO to create the lock if processe exits
                       before explicitly unlocking                     */
    };                  
static struct sembuf op_endcreate[2]=
    {
     {1,-1,SEM_UNDO}, /* derement [1] (process counter} with undo on exit */     
                    /* UNDO to adjust process counter if process exits
                       before explicitly calling sem_close()            */
     {2,-1,SEM_UNDO} /* then decrement [2] (lock) back to 0               */
    };
static struct sembuf op_open[1]=
    {
     {1,-1,SEM_UNDO} /* decrement [1] (process counter) with undo on exit */
    };
...
static struct sembuf op_unlock[1]=
    {
     {2,-1,SEM_UNDO} /* decrement [2] (lock) backt to 0                   */
    };    
static struct sembuf op_op[1]=
   {
    {0,99,SEM_UNDO} /* decrement or increment [0] with undo on exit       */
                  /* the 99 is set to actual amount to add 
                     or substruct (positive or negative)                */
   };
...
int sem_create(key_t key,int initial)
{
 ...
 if(key == IPC_PRIVATE)
    return(-1);          /* not intendet for private semaphores. */
 else if(key == (key_t) -1)    
...
/*
  ...
  from a client-server pair would us this, if its the server's
...
int sem_open(int key)
{
 ...
 if(key == IPC_PRIVATE)
    return(-1);  /* not intened for private semaphores */
 else if(key == (key_t) -1)
    return(-1);  /* probably an ftok() error by caller */
