...
char *pname;
/****************************************************************/
int get_telnet_response_code(char *buff,int length,int *response_code); 
...
/*
	...
	if(get_telnet_response_code(str,n,&retcode) == 1) {
...
{
	int sockfd,retval,port=PORT_TELNET,timeout=10;
	...
	if(argc < 2) {
		...
		fprintf(stderr,"port   : Is the port on which the server is listening. Default is %d and timeout is 10. These parameter are optional.\n",PORT_TELNET);
		fprintf(stderr,"Example: %s imap.example.org\n",argv[0]);
		exit(1);
	}
