...
void ConfFile::extractKeyValue(const char *str,string &key,string &value)
{
		...
		if(i != ls.end())
			value=(*i);
/*
	For now it does what I need, but change this function so that it can
	get strings with spaces for the value part.
	string s=str;
...
long int ConfFile::getLong(const char *key)
{
	return getLong(string(key));
}

long int ConfFile::getLong(const string key)
{
	string s=getKeyValue(key);
	if(s.size() == 0)
		return ConfFile::longNotFound;
...
void ConfFile::setFileName(const char *fileName)
{
	file=new ifstream;
	file->open(fileName);
	if(!file) { // I should throw an IO exception here
