...
class ConfFile
{
	public:
		...
		static const int longNotFound=LONG_MIN;
	public:
		//! Default constructor
		ConfFile();
		//! Constructor
		...
		long int getLong(const char *key);
		long int getLong(const string key);
	...
	private:
		...
		ConfFile(const ConfFile &) {};	// no copy constructor
		void operator=(const ConfFile &) {};	// no assignment operator
};
#endif