...
char *pname;
/****************************************************************/
int get_imap_response_code(char *buff,int length,int *response_code); 
...
int client(register int sockfd,int argc,char *argv[])
{
	...
	if(writen(sockfd,"A001 LOGOUT\n",12) != 12) {
		return(1);
	}
	if(get_imap_response_code(str,n,&retcode) == 1) {
...
{
	int sockfd,retval,port=PORT_IMAP;
