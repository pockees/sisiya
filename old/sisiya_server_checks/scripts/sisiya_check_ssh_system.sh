...
if test $# -lt $min_argc || test $# -gt $max_argc ; then
	echo "Usage : $0 sisiya_server_checks.conf check_system_name check_system port expire"
	echo "Usage : $0 sisiya_server_checks.conf check_system_name check_system port expire output_file"
	echo "Example: $0 sisiya_server_checks.conf system1.example.com 22 10" 
	echo "Example: $0 sisiya_server_checks.conf system1.example.com 22 10 output_file" 
...
##########################################################################
### sshd service id
serviceid=$serviceid_sshd  
if test -z "$serviceid" ; then
	echo "$0 : serviceid_sshd is not defined! Exiting..."
	exit 1
fi
##########################################################################
service_name="ssh"
check_prog=$sisiya_ssh_check_prog
