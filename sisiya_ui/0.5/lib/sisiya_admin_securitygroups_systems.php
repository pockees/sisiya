...
if($_SESSION['is_admin'] == 'f') 
       return;	
$html='';
$table_name='securitygroupsystem';
...
if(isset($_POST['add']))
	execSQLWrapper('add',"insert into ".$table_name." values(".$_POST['new_securitygroup_id'].",".$_POST['new_system_id'].")");
else if(isset($_POST['clear']))
	clearFields($formName,$fields);
else {
	for($i=0;$i<$nrows;$i++) {
		if(isset($_POST['delete'][$i])) 
			execSQLWrapper('delete','delete from '.$table_name.' where securitygroupid='.$_POST['securitygroup_id'][$i].' and systemid='.$_POST['system_id'][$i]);
	}
}
...
$securitygroups=getSQL2SelectArray("select a.id,c.str from securitygroups a,strkeys b,interface c,languages d where a.id > 0 and a.keystr=b.keystr and b.id=c.strkeyid and d.code='".$_SESSION['language']."' and d.id=c.languageid order by c.str");
...
$html.="</tr>";
#			0		1	2	3
$sql_str="select a.securitygroupid,a.systemid,i.str,c.hostname from securitygroupsystem a,securitygroups b,systems c,strkeys s,interface i,languages l";
$sql_str.=" where a.securitygroupid=b.id and a.systemid=c.id ";
